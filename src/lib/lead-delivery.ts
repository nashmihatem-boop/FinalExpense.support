 ...
 *
 * Requires RESEND_API_KEY (from resend.com). Sends from leads@finalexpense.support, which is a
...
async function sendLeadNotificationEmail(record: LeadRecord): Promise<EmailResult> {
  ...
  const html = `
    <div style="font-family: -apple-system, sans-serif; max-width: 480px;">
      <h2 style="color: #0e5265; margin-bottom: 4px;">New quote request</h2>
      <p style="color: #6b7280; margin-top: 0;">via FinalExpense.support</p>
  ...
  try {
    const { error } = await resendClient.emails.send({
      from: "FinalExpense.support Leads <leads@finalexpense.support>",
